...
class Game:
            ...
            if event.type == MOUSEBUTTONDOWN:
                if self.hop == False:
                    if self.board.location(
                            self.mouse_pos).occupant != None and self.board.location(
                ...
                if self.hop == True:
                    if self.selected_piece != None and self.mouse_pos in self.board.legal_moves(
                        ...
                        self.board.move_piece(self.selected_piece,
                                              self.mouse_pos)
                        self.board.remove_piece((self.selected_piece[0] + (
                                    self.mouse_pos[0] - self.selected_piece[
                                0]) / 2, self.selected_piece[1] + (
                                                             self.mouse_pos[
                                                                 1] -
                                                             self.selected_piece[
                                                                 1]) / 2))
    ...
    def check_for_endgame(self):
        ...
        for x in xrange(8):
            for y in xrange(8):
                if self.board.location(
                        (x, y)).color == BLACK and self.board.location(
                        (x, y)).occupant != None and self.board.location(
...
class Graphics:
    ...
    def pixel_coords(self, board_coords):
        ...
        """
        return (board_coords[0] * self.square_size + self.piece_size,
    ...
    def draw_message(self, message):
        ...
        self.text_surface_obj = self.font_obj.render(message, True, HIGH,
                                                     BLACK)
...
class Board:
    ...
    def blind_legal_moves(self, (x, y)):
        ...
        if self.matrix[x][y].occupant != None:
            ...
            if self.matrix[x][y].occupant.king == False and self.matrix[x][
                y].occupant.color == BLUE:
                blind_legal_moves = [self.rel(NORTHWEST, (x, y)),
                                     self.rel(NORTHEAST, (x, y))]

            elif self.matrix[x][y].occupant.king == False and self.matrix[x][
                y].occupant.color == RED:
    ...
    def legal_moves(self, (x, y), hop=False):
        ...
        if hop == False:
                    ...
                    if self.on_board(move):
                        if self.location(move).occupant == None:
                            legal_moves.append(move)
                                ...
                                move).occupant.color != self.location(
                                (x, y)).occupant.color and self.on_board((move[
                                                                              0] + (
                                                                                  move[
                                                                                      0] - x),
                                                                          move[
                                                                              1] + (
                                                                                  move[
                                                                                      1] - y))) and self.location(
                                (move[0] + (move[0] - x), move[1] + (move[
            ...
            for move in blind_legal_moves:
                if self.on_board(move) and self.location(
                        move).occupant != None:
                    if self.location(move).occupant.color != self.location(
                            (x, y)).occupant.color and self.on_board((move[
                                                                          0] + (
                                                                              move[
                                                                                  0] - x),
    ...
    def move_piece(self, (start_x, start_y), (end_x, end_y)):
        ...
        self.matrix[end_x][end_y].occupant = self.matrix[start_x][
            start_y].occupant
